...
const { convertResponse, getHomePath, getCowboyPath, hrtimeToStr } = require('./helpers');
...
module.exports = class Cowboy {
  ...
  constructor(options) {
    ...
    if (!this.options.poolId) {
      this.options.poolId = hrtimeToStr(TIMESTART);
    }

    const debugEnabled = !this.options.silent && !this.options.benchmark;
    this.state = new State(this.options.poolId, debugEnabled);
    this.dockerIsAvailable = this.isDockerAvailable();
    
    this.state.benchmark('booted');
  }
  ...
  execSync(cmd, cleanArgs = []) {
    ...
    try {
      this.state.benchmarkMark();

      if (execFile) {
        response = execFileSync(cmd0, cleanArgs);
        this.state.benchmark(`Command: ${cmd0}`);
      } else {
        response = execSync(cmd, this.execOptions);
        this.state.benchmark(`Command: ${cmd}`);
      }
  ...
  runThroughDocker(cmd, args = [], interactive = false) {
    ...
    // append the pool id for tracing
    cleanArgv.push('--pool-id');
    cleanArgv.push(this.options.poolId);
  ...
  getCleanArgs(slice = 3) {
    ...
    // Remove 2 pars
    const pos = cleanArgv.indexOf('--pool-id')
    if (pos > -1) {
      cleanArgv.splice(pos, 2);
    }

    // Filter out args to protect passing
    cleanArgv.filter((cleanArg) => {

      console.log(cleanArg);

      return cleanArg !== '--pool-id';
    })
