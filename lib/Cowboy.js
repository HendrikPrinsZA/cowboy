...
module.exports = class Main {
  ...
  async run(command) {
    ...
    if (extension.length > 0) {
      ...
      if (this.canRead(commandPathLocal)) {
        return this.runCommand(commandPathLocal);
      }

      if (this.canRead(commandPathPublic)) {
        return this.runCommand(commandPathPublic);
      }

      if (this.canRead(commandPathCore)) {
  ...
  runCommand(commandPath) {
    ...
    if (ext === '.rb') {
      return this.runCommandWithExec('ruby', commandPath, cleanArgv);
    }
